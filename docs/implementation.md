...
- **`supervisor.py`（sessiond）——会话所有权**：拥有全部 `PtySession` 生命周期；`detach()` 只断开 transport，绝不 kill PTY；显式 `terminate`、权威 `agents` 目录删除对应 agent 或 `shutdown()` 才结束 PTY。`agents` 帧必须是元素完整合法的列表才会原子替换目录（空列表是合法的 clear-all）；任一畸形元素使整帧保持 no-op。每次新 PTY 启动生成一个 UUID `pty_instance_id`，同一且仍存活的 PTY 的幂等 `open` 复用该值；如果子进程已被外部终止但 reader 尚未完成清理，`open` 会剔除 stale handle 并启动新实例。旧 reader 的迟到 `exit` 以对象身份校验隔离，不能误删或关闭替代它的新 PTY。
...
- 所有渲染进 HTML 的服务端字符串（name/handle/runtime…）经 `esc()` 转义。server 持久化 capability blob 时保持 opaque；当前 protocol 的 Web Add agent 表单只读取其顶层 string array，trim/去重并忽略畸形项。

### 4.3 `ui.js` + `ui.test.js`
DOM-free 的纯逻辑抽到 UMD 模块 `web/ui.js`：fleet 汇总、devbox/agent 过滤、
command 生成/筛选、runtime label/options、opaque agent API path、initials、状态映射、`escapeHtml`。`app.js` 用与
